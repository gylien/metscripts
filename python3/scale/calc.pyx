...
def interp_z(sio, np.ndarray[double, ndim=3] var, height=None, t=None, extrap=False):
...
def interp_p(sio, np.ndarray[double, ndim=3] var, plevels, p=None, t=None, bint extrap=False):
...
#        p = calc_pt(sio, tout=False, thetaout=False, t=t)[0]
    cdef np.ndarray[double, ndim=3] p_ = (calc_pt(sio, tout=False, thetaout=False, t=t)[0] if p is None else p)
...
#    varshape[0] = len(plevels)
    ...
    cdef np.ndarray[double, ndim=3] log_p = np.log(p_)
