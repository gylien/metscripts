...
def fort_seq_read(f, var):
    ...
    nbytes = struct.unpack(var.dtype.byteorder + 'i4', buf)[0]
    ...
    buf = f.read(4)
    if not buf: return False
    nbytes2 = struct.unpack(var.dtype.byteorder + 'i4', buf)[0]
    if nbytes != nbytes2:
...
def fort_seq_write(f, var):
    f.write(struct.pack(var.dtype.byteorder + 'i4', var.nbytes))
    var.tofile(f)
    f.write(struct.pack(var.dtype.byteorder + 'i4', var.nbytes))
    return True
